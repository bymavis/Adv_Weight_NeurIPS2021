...
#parser.add_argument('--load_adv_checkpoint', default='checkpoint/std_cifar20_train_resnet18/best_model_20.pth')
parser.add_argument('--load_cls_checkpoint', default='checkpoint/adv_beta1_cifar20_train_resnet18_cluster/best_model_20.pth')
...
parser.add_argument('--epsilon', '-e', type=float, default=2./255,
                    help='maximum perturbation of adversaries (4/255=0.0157)')
parser.add_argument('--alpha', '-a', type=float, default=1./255,
...
parser.add_argument('--adv', choices=['adv','std','cls'],default='std',help='what type of model to do: adv | std | trades')
...
class Trainer():
    ...
    def train(self, model, tr_loader, va_loader=None, adv_train=False):
        ...
        '''
        opt = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
        ...
        begin_time = time()
        best_pred = 0.0
        center=self.calculate_center(model,tr_loader)
        #print(center)
        ...
        for epoch in range(1, args.max_epoch+1):
            ...
            for data, label in tr_loader:
                ...
                for i in range(len(label)):
                    target.append(center[label[i]])
                target=torch.from_numpy(np.array(target))
                target.requires_grad=output.requires_grad
            ...
            if va_loader is not None:
                t1 = time()
                va_acc_20, va_adv_acc_20 = self.test(model, va_loader,)
    ...
    def calculate_center(self, model, loader):
        # adv_test is False, return adv_acc as -1 
        model.eval()

        feature=[[] for _ in range(5)]
        loss_fn = torch.nn.MSELoss(reduce=True, size_average=True)
        with torch.no_grad():
            for data,label in loader:
                data, label = tensor2cuda(data), tensor2cuda(label)
        ...
        #assert len(feature[0])==400,"size does not match"
        ...
        #center=np.array(center) #16*16
        return center

def main(args):
    save_folder = '%s_%s_finetune_fc_cifar20' % (args.dataset, args.affix)
    ...
    #model = vgg11_bn(num_classes=16)
    #model = DenseNet121(num_classes=16)
    if args.adv=='cls':
        model.load_state_dict(torch.load(args.load_cls_checkpoint))
    elif args.adv=='std':
        model.load_state_dict(torch.load(args.load_std_checkpoint))
    #resnet18 densenet inceptionv3
    num_ftr=model.linear.in_features
    model.linear=nn.Linear(num_ftr, 4) 
    #alexnet
    #num_ftr=model.last_layer.in_features
    #model.last_layer=nn.Linear(num_ftr, 5) 
    
    
    #vgg11 
    #num_ftr=model.classifier.in_features
    #model.classifier=nn.Linear(num_ftr, 4)
    ...
    train_root = 'data/cifar20/train_test_100'
    test_root = 'data/cifar20/test_100'

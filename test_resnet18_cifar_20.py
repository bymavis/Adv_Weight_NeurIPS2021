...
parser.add_argument('--load_std_checkpoint', default='checkpoint/std_cifar20_train_resnet18/best_model_20.pth')
parser.add_argument('--load_adv_checkpoint', default='checkpoint/adv_cifar20_train_resnet18/best_model_20.pth')
...
class Trainer():
    ...
    def pgd_whitebox(self, model, X, y, epsilon=8/255, num_steps=100, step_size=0.003):
        out = model(X)
    ...
    def test(self, model, loader, adv_test=False, use_pseudo_label=False):
        ...
        with torch.no_grad():
            for data, label in loader:
                ...
                te_acc = evaluate(np.floor(pred.cpu().numpy()/4), label.cpu().numpy(), 'sum')
                ...
                if adv_test:
                    ...
                    adv_pred = torch.max(adv_output, dim=1)[1]/4
...
def main(args):
    save_folder = '%s_test_resnet18_cifar16' % (args.affix)
...
#                                         download=True)
        ...
        if args.adv_train:
            ...
            checkpoint = torch.load(args.load_cls_checkpoint)
        else:
